<?php
   ...
   $OthersId = $_GET['id'];
   $query_edit = mysqli_query($db, "SELECT * from others where OthersId  = '$OthersId'");
    ...
    $State = mysqli_real_escape_string($db, $_POST["State"]);
       for ($i = 0;$i < count($_FILES["documents"]["name"]);$i++) {
           ...
           if ($_FILES["documents"]["name"][$i] != '') {
               ...
               if(move_uploaded_file($_FILES["documents"]["tmp_name"][$i], "../Others/" . $documents)){
        ...
        for ($k = 0;$k < count($_FILES["videos"]["name"]);$k++) {
           ...
           if ($_FILES["videos"]["name"][$k] != '') {
               ...
               if(move_uploaded_file($_FILES["videos"]["tmp_name"][$k], "../OthersVideos/" . $videos)){
               $query1 = mysqli_query($db, "INSERT INTO `others_videos` (`others_video_id`, `OthersId`, `VideoFile`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$OthersId', '$videos', '0', CURRENT_TIMESTAMP);");
             }}
           }
       }
       for ($j = 0;$j < count($_POST["YoutubeVideoLink"]);$j++) {
           ...
           if ($_POST["YoutubeVideoLink"][$j] != '') {
               $query1 = mysqli_query($db, "INSERT INTO `others_youtube` (`others_youtube_id`, `OthersId`, `YoutubeTitle`, `YoutubeTitleLink`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$OthersId', '', '$YoutubeVideoLink', '0', current_timestamp());");
           }
       }
           $query = mysqli_query($db, "UPDATE `others` SET `Title`='$Title', `Description`='$Description', `TrainingModulesTitle`='$TrainingModulesTitle', `TrainingModulesLink`='$TrainingModulesLink', `Remark`='$Remark', `DocumentsSource`='$DocumentsSource', `SourceLink`='$SourceLink', `State`='$State' WHERE `OthersId`='$OthersId'");
           if ($query) {
               header("Location:other-edits.php?id=$OthersId&msg=success");
           } else {
               header("Location:other-edits.php?id=$OthersId&msg=fail");
           }
   } ?>
<!DOCTYPE html>
<html lang="en">
               ...
               <div class="content-wrapper">
                  <div class="row">
                     <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                           <div class="card-body">
                              <h4 class="card-title float-left">Edit Others</h4>
