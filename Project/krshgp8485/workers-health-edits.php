...
$id = $_GET['id1'];
$query_edit = mysqli_query($db, "SELECT * from workers_health_safety_welfare where WorkersHealthSafetyWelfareId  = '$WorkersHealthSafetyWelfareId'");
...
if (isset($_POST["Title"])) {
    ...
    for ($i = 0;$i < count($_FILES["documents"]["name"]);$i++) {
        ...
        if ($_FILES["documents"]["name"][$i] != '') {
            ...
            if(move_uploaded_file($_FILES["documents"]["tmp_name"][$i], "../Workers-Health/" . $documents)){
    ...
    for ($j = 0;$j < count($_POST["YoutubeVideoLink"]);$j++) {
        ...
        if ($_POST["YoutubeVideoLink"][$j] != '') {
            $query1 = mysqli_query($db, "INSERT INTO `workers_health_safety_welfare_youtube` (`workers_health_safety_welfare_youtube_id`, `WorkersHealthSafetyWelfareId`, `YoutubeTitle`, `YoutubeTitleLink`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$WorkersHealthSafetyWelfareId', '', '$YoutubeVideoLink', '0', current_timestamp());");
        }
    }
     for ($i = 0;$i < count($_FILES["documents1"]["name"][$key]);$i++) {
        ...
        $query = mysqli_query($db, "UPDATE `workers_health_safety_welfare` SET `Title`='$Title', `Remark`='$Remark', `DocumentsSource`='$DocumentsSource', `SourceLink`='$SourceLink', `TrainingModulesTitle`='$TrainingModulesTitle', `TrainingModulesLink`='$TrainingModulesLink' WHERE `WorkersHealthSafetyWelfareId`='$WorkersHealthSafetyWelfareId'");
        if ($query) {
            header("Location:workers-health-edits.php?id=$WorkersHealthSafetyWelfareId&id1=$id&msg=success");
        } else {
            header("Location:workers-health-edits.php?id=$WorkersHealthSafetyWelfareId&id1=$id&msg=fail");
        }
} ?>
<!DOCTYPE html>
<html lang="en">
                     ...
                     <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                           <div class="card-body">
                              ...
                              <div class="card-title float-right"><a href="workers-health-edit1.php?id=<?php echo $id; ?>" class="btn btn-primary btn-sm">View List</a></div>
