<?php
   ...
   if (isset($_POST["others"])) {
       foreach ($_POST["others"] as $key => $other) {
             ...
             $query2 = mysqli_query($db, "INSERT INTO `others_si` (`OthersId`, `Title`, `Description`, `TrainingModulesTitle`, `TrainingModulesLink`, `Remark`, `DocumentsSource`, `SourceLink`, `State`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$Title', '$Description', '$TrainingModulesTitle', '$TrainingModulesLink', '$Remark', '$DocumentsSource', '$SourceLink', '$State', '0', current_timestamp());");
             $OthersId = $db->insert_id;
               if (!empty($YoutubeVideoLinks = $other['YoutubeVideoLink'])) {
                   foreach ($YoutubeVideoLinks as $YoutubeVideoLink) {
                       $query3 = mysqli_query($db, "INSERT INTO `others_youtube_si` (`others_youtube_id`, `OthersId`, `YoutubeTitleLink`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$OthersId', '$YoutubeVideoLink', '0', current_timestamp());");
                   }
               }
               for ($i = 0;$i < count($_FILES["documents"]["name"][$key]);$i++) {
                   ...
                   if ($_FILES["documents"]["name"][$key][$i] != '') {
                       ...
                       if(move_uploaded_file($_FILES["documents"]["tmp_name"][$key][$i], "../Others_SI/" . $documents)){
       ...
       for ($k = 0;$k < count($_FILES["documents1"]["name"][$key]);$k++) {
                   ...
                   if ($_FILES["documents1"]["name"][$key][$k] != '') {
                       ...
                       if(move_uploaded_file($_FILES["documents1"]["tmp_name"][$key][$k], "../OthersVideos_SI/" . $videos)){
                       $query12 = mysqli_query($db, "INSERT INTO `others_videos_si` (`others_video_id`, `OthersId`, `VideoFile`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$OthersId', '$videos', '0', current_timestamp());");
                     }}
                   }
               }        
       }
       header("Location:other-add_SI.php?msg=success");
   }
   ?>
<!DOCTYPE html>
<html lang="en">
               ...
               <div class="content-wrapper">
                  <div class="row">
                     <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                           <div class="card-body">
                              <h4 class="card-title float-left">Add Others (Sustainable Initiatives)
</h4>
