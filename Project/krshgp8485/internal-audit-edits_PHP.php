...
$query_edit = mysqli_query($db, "SELECT * from internal_audit_php where InternalAuditID  = '$InternalAuditID'");
$row_edit = mysqli_fetch_assoc($query_edit);
if (isset($_POST["GAPStandardWise"]) && isset($_POST["DocumentsTitle"]) && isset($_POST["VideoTitle"]) && isset($_POST["YoutubeVideoLink"]) && isset($_POST["TrainingModulesTitle"]) && isset($_POST["TrainingModulesLink"]) && isset($_POST["Remark"]) && isset($_POST["DocumentsSource"]) && isset($_POST["SourceLink"])) {
   $GAPStandardWise = mysqli_real_escape_string($db, $_POST["GAPStandardWise"]);
    ...
    $Document1 = time() . "-" . rand(1000, 9999) . "-DOC-" . $_FILES["Document"]["name"];
            ...
            if ($_FILES["Document"]["name"] != '') {
                ...
                if(move_uploaded_file($_FILES["Document"]["tmp_name"], "../Internal_audit_PHP/" . $Document)){
                    $querydoc = mysqli_query($db, "UPDATE `internal_audit_php` SET `Document`='$Document' WHERE `InternalAuditID`='$InternalAuditID'");
             }}
            }
        $query = mysqli_query($db, "UPDATE `internal_audit_php` SET `GAPStandardWise`='$GAPStandardWise', `DocumentsTitle`='$DocumentsTitle', `VideoTitle`='$VideoTitle', `YoutubeVideoLink`='$YoutubeVideoLink', `TrainingModulesTitle`='$TrainingModulesTitle', `TrainingModulesLink`='$TrainingModulesLink', `Remark`='$Remark', `DocumentsSource`='$DocumentsSource', `SourceLink`='$SourceLink' WHERE `InternalAuditID`='$InternalAuditID'");
        if ($query) {
            header("Location:internal-audit-edits_PHP.php?id=$InternalAuditID&msg=success");
        } else {
            header("Location:internal-audit-edits_PHP.php?id=$InternalAuditID&msg=fail");
        }
} ?>
<!DOCTYPE html>
<html lang="en">
                     ...
                     <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                           <div class="card-body">
                              ...
                              <div class="card-title float-right"><a href="internal-audit-edit_PHP.php" class="btn btn-primary btn-sm">View List</a></div>
