<?php
   ...
   $OthersId = $_GET['id'];
   $query_edit = mysqli_query($db, "SELECT * from others_si where OthersId  = '$OthersId'");
    ...
    $State = mysqli_real_escape_string($db, $_POST["State"]);
       for ($i = 0;$i < count($_FILES["documents"]["name"]);$i++) {
           ...
           if ($_FILES["documents"]["name"][$i] != '') {
               ...
               if(move_uploaded_file($_FILES["documents"]["tmp_name"][$i], "../Others_SI/" . $documents)){
               $query1 = mysqli_query($db, "INSERT INTO `others_documents_si` (`others_documents_id`, `OthersId`, `documents_name`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$OthersId', '$documents', '0', current_timestamp());");
             }}
           }
       }  
        for ($k = 0;$k < count($_FILES["videos"]["name"]);$k++) {
           ...
           if ($_FILES["videos"]["name"][$k] != '') {
               ...
               if(move_uploaded_file($_FILES["videos"]["tmp_name"][$k], "../OthersVideos_SI/" . $videos)){
               $query1 = mysqli_query($db, "INSERT INTO `others_videos_si` (`others_video_id`, `OthersId`, `VideoFile`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$OthersId', '$videos', '0', CURRENT_TIMESTAMP);");
             }}
           }
       }
       for ($j = 0;$j < count($_POST["YoutubeVideoLink"]);$j++) {
           ...
           if ($_POST["YoutubeVideoLink"][$j] != '') {
               $query1 = mysqli_query($db, "INSERT INTO `others_youtube_si` (`others_youtube_id`, `OthersId`, `YoutubeTitle`, `YoutubeTitleLink`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$OthersId', '', '$YoutubeVideoLink', '0', current_timestamp());");
           }
       }
           $query = mysqli_query($db, "UPDATE `others_si` SET `Title`='$Title', `Description`='$Description', `TrainingModulesTitle`='$TrainingModulesTitle', `TrainingModulesLink`='$TrainingModulesLink', `Remark`='$Remark', `DocumentsSource`='$DocumentsSource', `SourceLink`='$SourceLink', `State`='$State' WHERE `OthersId`='$OthersId'");
           if ($query) {
               header("Location:other-edits_SI.php?id=$OthersId&msg=success");
           } else {
               header("Location:other-edits_SI.php?id=$OthersId&msg=fail");
           }
   } ?>
<!DOCTYPE html>
<html lang="en">
               ...
               <div class="content-wrapper">
                  <div class="row">
                     <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                           <div class="card-body">
                              <h4 class="card-title float-left">Edit Others (Sustainable Initiatives)</h4>
                              <div class="card-title float-right"><a href="other-edit_SI.php" class="btn btn-primary btn-sm">View List</a></div>
