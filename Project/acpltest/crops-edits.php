<?php
   ...
   $CropId = $_GET['id'];
   $query_edit = mysqli_query($db, "SELECT * from crop where CropId = '$CropId'");
   $row_edit = mysqli_fetch_assoc($query_edit);
   if (isset($_POST["CropName"])) {
           $CropName = mysqli_real_escape_string($db, $_POST["CropName"]);
           $query = mysqli_query($db, "UPDATE `crop` SET `CropName` = '$CropName' WHERE `crop`.`CropId` = '$CropId'");
           if($query){
                header("Location:crops-edits.php?id=$CropId&msg=success");
           }else{
                header("Location:crops-edits.php?id=$CropId&msg=fail");
           }
   }
   ?>
<!DOCTYPE html>
               ...
               <div class="content-wrapper">
                  <div class="row">
                     <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                           <div class="card-body">
                              <h4 class="card-title float-left">Edit Crop</h4>
                                    ...
                                    <div class="row">
                                       <div class="col-12">
                                          <div class="form-group">
                                             ...
                                             <input value="<?php echo $row_edit['CropName']; ?>" name="CropName" type="text" class="form-control" id="" placeholder="Enter Crop Name" required="">
                                          </div>
                                       </div>
                                    </div>
                                 </div>
