...
if (isset($_POST["StandardCategory"]) AND isset($_POST["CropId"]) AND isset($_POST["GAPStandardWise"])) {
  ...
  $CropId = mysqli_real_escape_string($db, $_POST["CropId"]);
  $query2 = mysqli_query($db, "UPDATE `workers_health_safety_welfare_main` SET `StandardCategory`='$StandardCategory', `GAPStandardWise`='$GAPStandardWise', `CropId`='$CropId' WHERE `workers_health_safety_welfare_main_id`='$workers_health_safety_welfare_main_id'");
  if ($query2) {
         ...
         $SourceLink = mysqli_real_escape_string($db, $other["SourceLink"]);
            ...
            if (!empty($YoutubeVideoLinks = $other['YoutubeVideoLink'])) {
                foreach ($YoutubeVideoLinks as $YoutubeVideoLink) {
                    $query3 = mysqli_query($db, "INSERT INTO `workers_health_safety_welfare_youtube` (`workers_health_safety_welfare_youtube_id`, `WorkersHealthSafetyWelfareId`, `YoutubeTitleLink`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$WorkersHealthSafetyWelfareId', '$YoutubeVideoLink', '0', current_timestamp());");
                }
            }
            for ($i = 0;$i < count($_FILES["documents"]["name"][$key]);$i++) {
                ...
                if ($_FILES["documents"]["name"][$key][$i] != '') {
                    ...
                    if(move_uploaded_file($_FILES["documents"]["tmp_name"][$key][$i], "../Workers-Health/" . $documents)){
                    $query1 = mysqli_query($db, "INSERT INTO `workers_health_safety_welfare_documents` (`workers_health_safety_welfare_documents_id`, `WorkersHealthSafetyWelfareId`, `documents_name`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$WorkersHealthSafetyWelfareId', '$documents', '0', current_timestamp());");
                  }}
                }
            }
            for ($i = 0;$i < count($_FILES["documents1"]["name"][$key]);$i++) {
                ...
                if ($_FILES["documents1"]["name"][$key][$i] != '') {
                    ...
                    if(move_uploaded_file($_FILES["documents1"]["tmp_name"][$key][$i], "../Workers-Health-Images/" . $documents1)){
                    $query1 = mysqli_query($db, "INSERT INTO `workers_health_safety_welfare_images` (`workers_health_safety_welfare_images_id`, `WorkersHealthSafetyWelfareId`, `documents_name`, `DeletedStatus`, `CreatedDate`) VALUES (NULL, '$WorkersHealthSafetyWelfareId', '$documents1', '0', current_timestamp());");
                  }}
                }
            }
    ...
    header("Location:workers-health-edits1.php?id=$workers_health_safety_welfare_main_id&msg=success");
}
else{
    header("Location:workers-health-edits1.php?id=$workers_health_safety_welfare_main_id&msg=fail");
}
}
?>
<!DOCTYPE html>
               ...
               <div class="content-wrapper">
                  <div class="row">
                     <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                           <div class="card-body">
                              <h4 class="card-title float-left">Edit Workers Health, Safety & Welfare</h4>
                              <div class="card-title float-right"><a href="workers-health-edit.php" class="btn btn-primary btn-sm">View List</a></div>
