<?php
   ...
   error_reporting(0);
   if ($_SESSION['UID'] == '') {
    header("Location: login.php?page=SI_O"); 
   }
   ...
   if (!in_array('Sustainable Initiatives', explode(',',$_SESSION["PlanDetails"]))) {
    //   header("Location: login.php?page=SI_O");
       echo "<script>alert('You do not have permission to access this section')</script>";
        echo "<script>window.location.href='login.php?page=SI_O'</script>";
   }
...
<html lang="en">
   ...
   <body>
      ...
      <!--            <h1 class="display-3 text-white animated slideInDown" style="font-size: 45px;">Government Promoting Institutions (Sustainable Initiatives)</h1>-->
                  ...
                  <div class="row g-3 m-tab">
                     <div class="col-md-4">
                        <ul class="nav nav-tabs" id="myTab" role="tablist">
                           <?php
                              if($_GET['type'] != 'search'){
                                 $active1 ='active';
                                 $queryf = mysqli_query($db,"SELECT DISTINCT Title from others_si where DeletedStatus='0' ORDER BY Title");
                                 if($queryf){
                                    while($rowf = mysqli_fetch_array($queryf)){
                                       ?>
                           <li class="nav-item" role="presentation">
                              <a class="nav-link <?php echo $active1; ?>" href="other-option_SI.php?nm=<?php echo $rowf['Title']; ?>&type=search" aria-selected="true"><?php echo $rowf['Title']; ?></a>
                           </li>
                           <?php $active1 ='';}}}else{  
                              $queryf = mysqli_query($db,"SELECT DISTINCT Title from others_si where DeletedStatus='0' ORDER BY Title");
                              if($queryf){
                                 while($rowf = mysqli_fetch_array($queryf)){
                                    ?>
                           <li class="nav-item" role="presentation">
                              <a class="nav-link <?php if($_GET['nm'] == $rowf['Title']){echo "active";} ?> <?php echo $active1; ?>" href="other-option_SI.php?nm=<?php echo $rowf['Title']; ?>&type=search" aria-selected="true"><?php echo $rowf['Title']; ?></a>
                           </li>
                           <?php }}} ?>    
                        </ul>
                     </div>
                     <div class="col-md-8">
                        <?php if($_GET['type'] != 'search'){  }else{ 
                           $query11 = mysqli_query($db,"SELECT DISTINCT Title from others_si where DeletedStatus='0' AND Title = '$_GET[nm]' AND State != '0' ORDER BY Title LIMIT 1"); 
                        ...
                        <?php if($rownum == 1){ 
                           if($_GET['st'] == 'ALL'){
                                         $query = mysqli_query($db,"SELECT * from others_si where DeletedStatus='0' AND Title = '$Title' ORDER BY Title"); 
                                   }elseif($_GET['st'] !=''){
                                         $query = mysqli_query($db,"SELECT * from others_si where DeletedStatus='0' AND Title = '$Title' AND State = '$_GET[st]' ORDER BY Title"); 
                                   }else{
                                    $query = mysqli_query($db,"SELECT * from others_si where DeletedStatus='0' AND Title = '$Title' ORDER BY Title"); 
                                   }
                                   $rownums = mysqli_num_rows($query);
                           if($rownums == '0'){
                                         $query11 = mysqli_query($db,"SELECT * from others_si where DeletedStatus='0' AND Title = '$Title' ORDER BY Title"); 
                                         $num11 = 0;
                           ...
                           <div class="form-group">
                              <div class="input-group col-xs-4">
                                 <select name="State" class="form-control" style="color: #282f3a;" onchange="if (this.value) window.location.href=this.value">
                                    ...
                                    <option value="other-option_SI.php?nm=$Title&type=search">Select State</option>
                                    <?php }  ?>
                                    <?php
                                       $querycl = mysqli_query($db,"SELECT DISTINCT State from others_si where Title = '$Title' AND DeletedStatus='0' AND State != '0' ORDER BY State"); 
                                          while($rowcl = mysqli_fetch_array($querycl)){
                                       ?>
                                    <option <?php if($_GET['st'] == $rowcl['State']){echo "selected";}  ?> value="other-option_SI.php?nm=<?php echo $_GET['nm'];?>&type=search&st=<?php echo $rowcl['State'];?>"><?php echo $rowcl['State'];?></option>
                                    <?php } ?> 
                                 </select>
                              </div>
                           </div>
                        ...
                        <div class="col-4" style="padding-bottom: 10px;">
                           <div class="form-group">
                              <div class="input-group col-xs-4">
                                 <select name="State" class="form-control" style="color: #282f3a;" onchange="if (this.value) window.location.href=this.value">
                                    <option value="other-option_SI.php?nm=<?php echo $Title;?>&type=search">Select State</option>
                                    <option <?php if($_GET['st'] == 'ALL'){echo "selected";}  ?> value="other-option_SI.php?nm=<?php echo $Title;?>&type=search&st=ALL">Select ALL</option>
                                    <?php
                                       $querycl = mysqli_query($db,"SELECT DISTINCT State from others_si where Title = '$Title' AND DeletedStatus='0' AND State != '0' ORDER BY State"); 
                                          while($rowcl = mysqli_fetch_array($querycl)){
                                       ?>
                                    <option <?php if($_GET['st'] == $rowcl['State']){echo "selected";}  ?> value="other-option_SI.php?nm=<?php echo $Title;?>&type=search&st=<?php echo $rowcl['State'];?>"><?php echo $rowcl['State'];?></option>
                                    <?php } ?> 
                                 </select>
                              </div>
                           </div>
                        ...
                        <div class="row">
                           <?php
                              $numdt = '1';
                                 $query22num = mysqli_query($db, "SELECT * from others_documents_si where DeletedStatus='0' and OthersId='$row[OthersId]'");
                           ...
                           <?php }  
                              if ($numdt > 0 AND $_GET['st'] != '') {
                                 $query1 = mysqli_query($db,"SELECT * from others_documents_si where DeletedStatus='0' and OthersId='$row[OthersId]' ORDER BY documents_name");
                                    if($query1){
                                      $docnum2 = 1;
                           ...
                           <div class="col-md-2">
                              ...
                              <a href="other-option-pdf_SI.php?doc=<?php echo $row1['others_documents_id']; ?>" target="_blank"><i style="font-size: 30px;" class="ti-files btn-icon-prepend"></i></a>
                              <?php }else{?>
                              <a href="Others_SI/<?php echo $row1['documents_name']; ?>" target="_blank"><i style="font-size: 30px;" class="ti-files btn-icon-prepend"></i></a>
                              <?php } ?>
                           </div>
                           <div class="col-md-3">
                           ...
                           <div class="col-md-12">
                              <?php
                                 $numdt1 = '0';
                                    $query22num1 = mysqli_query($db, "SELECT * from others_youtube_si where DeletedStatus='0' and OthersId='$row[OthersId]' and YoutubeTitleLink !=''");
                                    $numdt1+= mysqli_num_rows($query22num1);
                                    if ($numdt1 > 0) {
                                       ?>
                              <h5 style="padding-top: 8px;">You tube links</h5>
                              <?php }
                                 $query1 = mysqli_query($db,"SELECT * from others_youtube_si where DeletedStatus='0' and OthersId='$row[OthersId]' and YoutubeTitleLink !='' ORDER BY others_youtube_id ASC");
                                 if($query1){
                                    $docnum1 = 1;
                                    while($row1 = mysqli_fetch_array($query1)){
                                       ?>
                           ...
                           <div class="col-md-12">
                              <?php
                                 $numdt11 = '0';
                                    $query22num11 = mysqli_query($db, "SELECT * from others_videos_si where DeletedStatus='0' and OthersId='$row[OthersId]'");
                                    $numdt11+= mysqli_num_rows($query22num11);
                                    if ($numdt11 > 0) {
                                       ?>
                              <h5 style="padding-top: 8px;">Videos</h5>
                              <?php }
                                 $query15 = mysqli_query($db,"SELECT * from others_videos_si where DeletedStatus='0' and OthersId='$row[OthersId]' ORDER BY others_video_id ASC");
                                 if($query15){
                                    $docnum15 = 1;
                                    while($row15 = mysqli_fetch_array($query15)){
                                       ?>
                              <a href="OthersVideos_SI/<?php echo $row15['VideoFile']; ?>" target="_blank">
                                 <video height="200" width="200" controls>
                                    <source src="OthersVideos_SI/<?php echo $row15['VideoFile']; ?>" type="video/mp4">
                                 </video>
                              </a>
