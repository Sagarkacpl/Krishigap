<?php
   ...
   error_reporting(0);
   if ($_SESSION['UID'] == '') {
    header("Location: login.php?page=OFP_O"); 
   }
   ...
   if (!in_array('On Farm Production', explode(',',$_SESSION["PlanDetails"]))) {
    //   header("Location: login.php?page=OFP_O");
       echo "<script>alert('You do not have permission to access this section')</script>";
        echo "<script>window.location.href='login.php?page=OFP_O'</script>";
   }
...
<html lang="en">
   ...
   <body>
      ...
      <!--            <h1 class="display-3 text-white animated slideInDown">Government Promoting Institutionss</h1>-->
