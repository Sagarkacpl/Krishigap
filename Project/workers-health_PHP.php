<?php
   ...
   error_reporting(0);
   if ($_SESSION['UID'] == '') {
     header("Location: login.php?page=PHP_WH"); 
   }
   ...
   if (!in_array('Post Harvest Processing', explode(',',$_SESSION["PlanDetails"]))) {
    //   header("Location: login.php?page=PHP_WH");
       echo "<script>alert('You do not have permission to access this section')</script>";
        echo "<script>window.location.href='login.php?page=PHP_WH'</script>";
   }
   ...
   if ($_GET['sc'] != '' AND $_GET['gsw'] != '') {
       ...
       if ($_GET['gsw'] == 'BRCGlobalStandardFoodSafety9') {
           ...
           $query21 = mysqli_query($db, "SELECT * from workers_health_safety_welfare_main_php WHERE StandardCategory = '$_GET[sc]' AND GAPStandardWise = '$GETgsw' AND DeletedStatus='0'");
           $query21num = mysqli_num_rows($query21);
           $query211 = mysqli_query($db, "SELECT * from workers_health_safety_welfare_main_php WHERE StandardCategory = '$_GET[sc]' AND GAPStandardWise = '$GETgsw' AND DeletedStatus='0'");
           $query2111 = mysqli_query($db, "SELECT * from workers_health_safety_welfare_main_php WHERE StandardCategory = '$_GET[sc]' AND GAPStandardWise = '$GETgsw' AND DeletedStatus='0'");
           $query21num = mysqli_query($db, "SELECT * from workers_health_safety_welfare_main_php WHERE StandardCategory = '$_GET[sc]' AND GAPStandardWise = '$GETgsw' AND DeletedStatus='0'");
...
<html lang="en">
   ...
   <body>
      ...
      <!--            <h1 class="display-3 text-white animated slideInDown" style="font-size: 45px;">Workers health, safety & welfare (Post Harvest Processing)</h1>-->
            ...
            <form action="" method="GET">
               <div class="row g-3">
                  ...
                  <div class="col-md-5">
                     ...
                     <div class="form-floating">
                        <select name="sc" class="form-control" required="">
                           <option></option>
                           <option <?php if($_GET['sc'] == 'Post Harvest Processing'){echo "selected";} ?> value="Post Harvest Processing">Post Harvest Processing</option>
                        </select>
                     </div>
                  </div>
                  ...
                  <div class="row g-3" style="margin-top: 0px;">
                     <div class="col-md-2">
                        <h5>&nbsp;</h5>
                        <a href="workers-health_about.php" target="_blank" style="color: #fff;" class="btn btn-info w-100 py-3" type="submit">About This Section</a>
                     </div>
                     <!--<div class="col-md-2">-->
                     <!--   <h5>&nbsp;</h5>-->
                     <!--   <a href="workers-health-related-documents_PHP.php" target="_blank" style="color: #fff;" class="btn btn-info w-100 py-3" type="submit">Related Documents</a>-->
                     <!--</div>-->
                  </div>
                  <br>
                  ...
                  <div class="row g-3 m-tab">
                     <?php
                        while ($row21num = mysqli_fetch_array($query21num)) {
                            $query3num = mysqli_query($db, "SELECT * FROM `workers_health_safety_welfare_php`  WHERE workers_health_safety_welfare_main_id = '$row21num[workers_health_safety_welfare_main_id]' AND DeletedStatus='0'");
                            while ($row3num = mysqli_fetch_array($query3num)) {
                                $query4num = mysqli_query($db, "SELECT * FROM `workers_health_safety_welfare_documents_php`  WHERE WorkersHealthSafetyWelfareId = '$row3num[WorkersHealthSafetyWelfareId]' AND DeletedStatus='0'");
                                $num = 0;
                     ...
                     <?php } $docnum = 1;
                        ...
                        $query3 = mysqli_query($db, "SELECT * FROM `workers_health_safety_welfare_php`  WHERE workers_health_safety_welfare_main_id = '$row21[workers_health_safety_welfare_main_id]' AND DeletedStatus='0'");
                                 while ($row3 = mysqli_fetch_array($query3)) {
                        $query4 = mysqli_query($db, "SELECT * FROM `workers_health_safety_welfare_documents_php`  WHERE WorkersHealthSafetyWelfareId = '$row3[WorkersHealthSafetyWelfareId]' AND DeletedStatus='0'");
                     ...
                     <div class="col-md-2">
                        ...
                        <a href="workers-health-pdf_PHP.php?doc=<?php echo $row4['workers_health_safety_welfare_documents_id']; ?>" target="_blank"><i style="font-size: 30px;" class="ti-files btn-icon-prepend"></i></a>
                        <?php }else{?>
